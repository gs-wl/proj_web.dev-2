...
export const useNewsCache = () => {
  ...
  const saveCachedData = useCallback(async (twitterPosts: any[], aiNews: any[]) => {
    try {
      const response = await fetch('/api/news/cache', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ twitterPosts, aiNews }),
      });
  ...
  const checkForUpdates = useCallback(async (currentTwitter: any[], currentNews: any[]) => {
    try {
      ...
      if (newTwitterPosts.length > 0 || newNewsArticles.length > 0) {
        setNotification({
          hasNewUpdates: true,
