      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          { 
            ...
            from: 'from-green-600 to-emerald-600',
          ...
          { 
            ...
            from: 'from-orange-600 to-red-600',
            action: 'Start Farming'
          },
        ].map(card => (
          <div key={card.title} className={`bg-gradient-to-r ${card.from} rounded-xl p-6 text-white hover:shadow-xl transition-all duration-300 cursor-pointer group`}>
            <div className="flex items-center justify-between mb-4">
              <card.icon className="w-8 h-8 group-hover:scale-110 transition-transform duration-300" />
              <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform duration-300" />
            </div>
            <h3 className="text-xl font-bold mb-2">{card.title}</h3>
            <p className="mb-4 opacity-90">{card.desc}</p>
            <button className="bg-white/95 hover:bg-white text-gray-800 hover:text-gray-900 px-4 py-2 rounded-lg font-semibold transition-all backdrop-blur-sm group-hover:scale-105">
              {card.action}
            </button>
          </div>
        ))}
